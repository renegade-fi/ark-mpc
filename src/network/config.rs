...
use quinn::{ClientConfig, ServerConfig, TransportConfig, IdleTimeout, VarInt};
...
> 
{
    ...
    transport_config.max_idle_timeout(Some(
        IdleTimeout::from(VarInt::from_u32(MAX_IDLE_TIMEOUT))
    ));

    transport_config.keep_alive_interval(Some(
        Duration::from_millis(KEEP_ALIVE_INTERVAL)       
    ));
    ...
    let (cert, key) = generate_cert()
        .map_err( |_| SetupError::KeygenError )?;
    ...
    let mut client_crypto_config = CryptoClientConfig::builder()
        ...
        .with_no_client_auth();
    client_crypto_config.dangerous()
