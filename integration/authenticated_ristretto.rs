...
use crate::{
    mpc_ristretto::is_equal_u64, mpc_scalar::PartyIDBeaverSource, IntegrationTest,
    IntegrationTestArgs,
};
...
fn test_add(test_args: &IntegrationTestArgs) -> Result<(), String> {
    ...
    if shared_public.visibility() != Visibility::Shared {
        return Err(format!(
            "Expected visibility {:?}, got {:?}",
            Visibility::Shared,
            shared_public.visibility()
        ));
    }

    let shared_public_open = shared_public
        .open_and_authenticate()
        .map_err(|err| format!("Error opening and authenticating: {:?}", err))?;
    if !is_equal_u64(shared_public_open.to_ristretto(), 12) {
        return Err(format!(
            "Expected {}, got {:?}",
            12,
            shared_public_open.to_ristretto()
        ));
    }
    ...
    let public_public_open = public_public
        .open_and_authenticate()
    ...
    if !is_equal_u64(public_public_open.to_ristretto(), 14) {
        return Err(format!(
            "Expected {}, got {:?}",
            14,
            public_public_open.to_ristretto()
        ));
    }
